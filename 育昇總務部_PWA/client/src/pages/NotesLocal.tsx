...
import { notesDB, type Note, type Folder as FolderType, type Tag as TagType } from '@/lib/db';
import { useUITexts } from '@/contexts/UITextsContext';

export default function NotesLocal() {
  ...
  const fetchNotes = async () => {
    try {
      ...
      if (searchQuery) {
        allNotes = await notesDB.searchNotes(searchQuery);
      } else {
      ...
      if (selectedFolder !== null) {
        allNotes = allNotes.filter(note => note.folderId === selectedFolder);
      }

      // 篩選標籤
      if (selectedTag !== null) {
        allNotes = allNotes.filter(note => note.tags && note.tags.includes(String(selectedTag)));
      }
  ...
  const fetchFolders = async () => {
    try {
      const allFolders = await notesDB.getAllFolders();
  ...
  }, [searchQuery, selectedFolder, selectedTag, loading]);

  const createNewNote = () => {
    navigate(`${basePath}/notes/new`);
  };
    ...
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="p-2 border-b">
        <Button
          ...
          onClick={() => navigate(`${basePath}/notes`)}
        >
          {texts.categoryNotes}
        </Button>
        <Button
        ...
        >
          {texts.categoryInvestment}
        </Button>
        <Button
          variant="ghost"
          className="w-full justify-start mb-1"
          onClick={() => navigate(isSyncMode ? '/notes' : '/sync/notes')}
        >
          {isSyncMode ? '切換到本機版（離線）' : '切換到同步版（Google 備份）'}
        </Button>
      </div>
      <div className="p-4 border-b">
      ...
      <ScrollArea className="flex-1">
        ...
        <div className="p-2 border-t mt-2">
          <h3 className="text-sm font-semibold mb-2 px-2">{texts.tags}</h3>
          {tags.map((tag) => (
            <Button
              key={tag.id}
        ...
        <div className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-semibold">{texts.appTitle}</h1>
            </div>
        ...
        <ScrollArea className="flex-1">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full p-8 text-center">
              <p className="text-muted-foreground mb-4">{texts.noNotes}</p>
              <Button onClick={createNewNote}>
                <Plus className="mr-2 h-4 w-4" />
                {texts.createFirstNote}
              </Button>
            </div>
          ) : (
